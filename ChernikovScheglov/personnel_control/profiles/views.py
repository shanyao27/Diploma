...
from main_page.models import User, Admin, Inspector
from datetime import timedelta
from .models import (
    WorkObject, CertificationTest, UserTestAttempt, UserBlock, Document,
    DocumentSignature, ViolationSnapshot, MedicalCheck, SickLeave, Sanction, Department, VideoInspection
)
from .forms import (
    WorkObjectForm, CertificationTestForm, QuestionForm, DocumentForm,
    MedicalCheckForm, SickLeaveForm, SanctionForm
)
...
def _get_current_employee(request):
    if request.session.get('user_role') not in ['user', 'medic']:
        return None
...
def admin_users_list(request):
    """Список сотрудников для администратора с учетом участка."""
    _sync_reference_statuses()
    status_filter = request.GET.get('status', 'all')
    admin = _get_current_admin(request)
    users = _department_filtered_users(admin).select_related('department', 'position', 'manager')
...
def admin_user_detail(request, user_id):
    ...
    return render(request, 'profiles/admin/user_detail.html', {'user': user, 'admin_obj': admin})


def admin_user_confirm(request, user_id):
    """Подтверждение пользователя (isActive = True) и назначение всех тестов"""
    admin = _get_current_admin(request)
    user = get_object_or_404(_department_filtered_users(admin), id=user_id)
    user.isActive = True
    user.employment_status = 'active'
    if not user.hire_date:
        user.hire_date = timezone.localdate()

    from .models import CertificationTest
    tests = CertificationTest.objects.all()
    assigned_count = 0
...
def confirm_users(request):
    """Подтверждение выбранных пользователей"""
    if request.method == 'POST':
        ...
        if user_ids:
            ...
            messages.success(request,
                             f'Подтверждено пользователей: {len(user_ids)}, назначено тестов: {total_assigned}')
        else:
...
def admin_workobject_create(request):
    ...
    if request.method == 'POST':
        form = WorkObjectForm(request.POST, current_admin=current_admin)
        if form.is_valid():
            work_object = form.save(commit=False)

            if not current_admin.is_global_admin and current_admin.department:
                work_object.department = current_admin.department

            if hasattr(work_object, 'responsible_admin') and not work_object.responsible_admin:
                work_object.responsible_admin = current_admin

            work_object.save()
            form.save_m2m()

            messages.success(request, f'Наряд #{work_object.id} успешно создан')
            return redirect('profiles:admin_workobjects_list')
    else:
        form = WorkObjectForm(current_admin=current_admin)
...
def admin_workobjects_list(request):
    ...
    if status_filter == 'recruiting':
        work_objects = work_objects.filter(status='recruiting', time__gt=timezone.now())
    elif status_filter == 'completed':
        work_objects = work_objects.filter(Q(status='completed') | Q(time__lte=timezone.now()))
    elif status_filter == 'staffed':
        work_objects = work_objects.filter(employeesReady=True, time__gt=timezone.now())
...
def admin_medical_checks_list(request):
    _sync_reference_statuses()
    admin = _get_current_admin(request)
    checks = MedicalCheck.objects.select_related('employee', 'medic').order_by('-checked_at')
    if admin and not admin.is_global_admin and admin.department_id:
        checks = checks.filter(employee__department_id=admin.department_id)
    return render(request, 'profiles/admin/medical_checks_list.html', {'checks': checks, 'admin_obj': admin})
...
def admin_medical_check_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            check.check_date = timezone.localtime(check.checked_at).date()
            check.save()

            if check.decision == 'fit':
                check.employee.employment_status = 'active'
            elif check.decision == 'unfit':
    ...
    return render(request, 'profiles/admin/medical_check_form.html',
                  {'form': form, 'title': 'Новый медосмотр', 'admin_obj': admin})

def admin_sick_leaves_list(request):
    _sync_reference_statuses()
    admin = _get_current_admin(request)
    sick_leaves = SickLeave.objects.select_related('employee', 'issued_by').order_by('-start_date')
    if admin and not admin.is_global_admin and admin.department_id:
        sick_leaves = sick_leaves.filter(employee__department_id=admin.department_id)
    return render(request, 'profiles/admin/sick_leaves_list.html', {'sick_leaves': sick_leaves, 'admin_obj': admin})
...
def admin_sick_leave_create(request):
    ...
    return render(request, 'profiles/admin/sick_leave_form.html',
                  {'form': form, 'title': 'Новый больничный', 'admin_obj': admin})
...
def admin_sanction_create(request):
    ...
    return render(request, 'profiles/admin/sanction_form.html',
                  {'form': form, 'title': 'Новая санкция', 'admin_obj': admin})
...
def user_dashboard(request):
    ...
    if not user.isActive:
        return render(request, 'profiles/user/pending_approval.html', {
            'user_login': user_login
        })

    from profiles.models import CertificationTest
    pending_tests = CertificationTest.objects.filter(id__in=user.assigned_certifications)

    if user.has_pending_certifications:
        messages.warning(
            request,
            f'У вас есть непройденные аттестации: {", ".join([t.title for t in pending_tests])}'
        )
...
def medic_medical_checks_list(request):
    ...
    morning_passed = checks.filter(
        check_date=today,
        check_type='morning'
    ).values('employee_id').distinct().count()

    evening_total = employees.count()
    evening_passed = checks.filter(
        check_date=today,
        check_type='evening'
    ).values('employee_id').distinct().count()

    morning_missed = morning_total - morning_passed
    evening_missed = evening_total - evening_passed
...
def medic_medical_check_detail(request, check_id):
    ...
    return render(request, 'profiles/medic/medical_check_detail.html', {
        'check': check,
    })
...
def medic_medical_check_create_for_employee(request, employee_id):
    ...
    medic_user = _get_current_employee(request)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not getattr(check, 'check_date', None):
                if getattr(check, 'checked_at', None):
                    check.check_date = timezone.localtime(check.checked_at).date()
                else:
                    check.check_date = timezone.localdate()
    ...
    if user_role == 'user':
        try:
            ...
            if user.active_sick_leave:
                deny_reasons.append(
                    f'Сотрудник находится на больничном до {user.active_sick_leave.end_date.strftime("%d.%m.%Y")}.'
                )
            else:
                today_check = user.todays_medical_check
                if today_check:
                    if getattr(today_check, 'decision', None) == 'unfit':
        ...
        medic = User.objects.filter(id=user_id, system_role='medic').first()
        if not medic:
            return redirect('main_page:login')
...
def inspector_video_inspection_create(request):
    ...
    from .forms import VideoInspectionForm
    from .ppe_video import run_inspection
    import threading

    if request.method == 'POST':
        form = VideoInspectionForm(request.POST, request.FILES)
        if form.is_valid():
            obj = form.save(commit=False)
            work_obj = form.cleaned_data.get('work_object')
            obj.work_object = work_obj

            # Номер объекта формируем из наряда
            # В твоей модели WorkObject есть address
            obj.object_number = f"Наряд #{work_obj.id} — {work_obj.address}"
            if work_obj:
                obj.work_object = work_obj
                # чтобы везде красиво показывалось, можно заполнить object_number
                obj.object_number = f"Наряд #{work_obj.id} — {work_obj.address}"

            inspector_id = request.session.get('user_id')
            try:
                obj.inspector = Inspector.objects.get(id=inspector_id)
            except Exception:
                obj.inspector = None
            ...
            t = threading.Thread(target=run_inspection, args=(obj.id,), daemon=True)
            t.start()

            messages.success(request, 'Видео принято. Идёт обработка...')
            return redirect('profiles:inspector_video_inspection_detail', pk=obj.id)
    else:
        form = VideoInspectionForm()

    return render(request, 'profiles/inspector/video_inspection_create.html', {
        'form': form
    })
...
def inspector_video_inspection_detail(request, pk: int):
    ...
    if inspection.work_object:
        employees = inspection.work_object.employees.all()
...
def inspector_violation_decision(request, pk: int, violation_id: int):
    ...
    if inspector and inspector.department_id:
        inspections_qs = inspections_qs.filter(work_object__department_id=inspector.department_id)
    inspection = get_object_or_404(inspections_qs, id=pk)
    violation = get_object_or_404(ViolationSnapshot, id=violation_id, inspection=inspection)

    if request.method == 'POST':
        employee_id = request.POST.get('employee_id')
        if employee_id:
            violation.employee_id = int(employee_id)
        else:
            violation.employee = None
        decision = request.POST.get('decision')  # yes / no
        ...
        violation.save(update_fields=['confirmed', 'comment', 'reviewed_at', 'employee'])
...
def user_current_works(request):
    """Список нарядов, на которые записан пользователь"""
    user_id = request.session.get('user_id')
    ...
    if status_filter == 'active':
        my_works = my_works.filter(status__in=['recruiting', 'waiting'])
    elif status_filter == 'completed':
        my_works = my_works.filter(status='completed')

    return render(request, 'profiles/user/current_works.html', {
        'my_works': my_works,
        'user': user,
        'now': timezone.now()
    })
...
def user_available_works(request):
    """Список доступных нарядов для сотрудника"""
    user_id = request.session.get('user_id')
    ...
    if not user.AllowedToWork:
        if user.has_pending_certifications:
            ...
            messages.warning(
                request,
                f'Для записи на наряды необходимо пройти аттестации: {test_names}'
            )
        elif not user.noDocuments:
            messages.warning(
                request,
                'Для записи на наряды необходимо подписать все документы'
            )
        return redirect('profiles:dashboard')
    ...
    return render(request, 'profiles/user/available_works.html', {
...
def user_signup_work(request, work_id):
    ...
    if active_works.exists():
        ...
        messages.warning(
            request,
            f'Вы уже записаны на наряд #{active_work.id} ({active_work.address}). '
            f'Отпишитесь от него, чтобы записаться на новый.'
        )
        return redirect('profiles:user_current_works')

    work.employees.add(user)
    work.last_signup_time = timezone.now()
    work.save()
...
def admin_workobject_assign_lead(request, pk):
    """Назначение старшего сотрудника на наряд (только для админа)"""
    user_role = request.session.get('user_role')
...
def admin_test_list(request):
    """Список тестов для админа/инспектора"""
    ...
    if user_role == 'admin':
        admin_user = Admin.objects.get(id=user_id)
        tests = CertificationTest.objects.filter(created_by_admin=admin_user)
    else:
        inspector_user = Inspector.objects.get(id=user_id)
        tests = CertificationTest.objects.filter(created_by_inspector=inspector_user)

    return render(request, 'profiles/admin/test_list.html', {
        'tests': tests,
        'user_role': user_role
    })
...
def admin_test_create(request):
    ...
    if request.method == 'POST':
        form = CertificationTestForm(request.POST)
        if form.is_valid():
            ...
            if user_role == 'admin':
                ...
                test.created_by_inspector = Inspector.objects.get(id=user_id)
        ...
        form = CertificationTestForm()

    return render(request, 'profiles/admin/test_form.html', {
        'form': form,
...
def admin_assign_all_tests_to_all_users(request):
    """Назначить все тесты всем пользователям"""
    user_role = request.session.get('user_role')
    ...
    messages.success(request, f'Все тесты назначены всем пользователям (всего {total_assigned} назначений)')
    return redirect('profiles:admin_users_list')


def user_test_list(request):
    """Список доступных тестов для пользователя"""
    user_id = request.session.get('user_id')
    user_role = request.session.get('user_role')
    ...
    active_block = UserBlock.objects.filter(user=user, expires_at__gt=timezone.now()).first()
    if active_block:
        days_left = (active_block.expires_at - timezone.now()).days + 1
...
def user_take_test(request, test_id):
    """Прохождение теста: вопрос за вопросом. Ошибка = тест завершён."""
    user_id = request.session.get('user_id')
    user_role = request.session.get('user_role')
    ...
    # ---- Состояние прохождения теста в session ----
    ...
    # пробуем достать текущую попытку из сессии
    ...
    # если нет активной попытки — создаём
    if attempt is None:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # если ошибка — тест завершён + блокировка
            if not is_correct:
            ...
            # если это был последний вопрос — тест сдан
            if qidx >= total:
                attempt.passed = True
                attempt.completed_at = timezone.now()
                attempt.save(update_fields=['passed', 'completed_at'])

                # снимаем тест из назначенных
...
def admin_workobject_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            obj.save()
            form.save_m2m()
            messages.success(request, f'Наряд #{work_object.id} успешно обновлен')
            return redirect('profiles:admin_workobjects_list')
    else:
    ...
    return render(request, 'profiles/admin/workobject_form.html', {
        'form': form,
        'title': f'Редактирование наряда #{work_object.id}',
        'admin_obj': admin,
...
def admin_document_create(request):
    """Создание документа и рассылка пользователям своего участка или всем (для глобального)."""
    user_role = request.session.get('user_role')
    user_id = request.session.get('user_id')
    admin = _get_current_admin(request)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            from main_page.models import Admin
            ...
            messages.success(
                request,
                f'Документ "{document.title}" создан и разослан {users.count()} пользователям'
            )
...
def admin_document_list(request):
    """Список всех документов для админа"""
    user_role = request.session.get('user_role')
...
def user_documents(request):
    """Список документов для пользователя (НЕПОДПИСАННЫЕ + ПОДПИСАННЫЕ)"""
    user_id = request.session.get('user_id')
    ...
    all_assigned_docs = Document.objects.filter(
        id__in=user.documents,
        is_active=True
    ).order_by('-created_at')
...
def sign_document(request, doc_id):
    ...
    if request.method == 'POST':
        ...
        check_user_documents_completion(user)

        messages.success(request, f'Документ "{document.title}" подписан')
        return redirect('profiles:user_documents')

    return render(request, 'profiles/user/sign_confirm.html', {
        'document': document,
        'user': user
    })


def check_user_documents_completion(user):
    """Проверяет, все ли документы подписаны пользователем"""
    assigned_doc_ids = user.documents

    if not assigned_doc_ids:
        print("Нет назначенных документов")
        return True

    from .models import DocumentSignature
    signed_doc_ids = list(DocumentSignature.objects.filter(
        user=user,
        document_id__in=assigned_doc_ids
    ).values_list('document_id', flat=True))

    if set(assigned_doc_ids) == set(signed_doc_ids):
        print("Все документы подписаны")
        return True
    else:
        print("Не все документы подписаны")
        return False
...
def user_violations(request):
    ...
    violations = (ViolationSnapshot.objects
                  .filter(employee_id=user_id, confirmed=True)
                  .select_related('inspection', 'inspection__work_object')
                  .order_by('-created_at'))

    return render(request, 'profiles/user/violations.html', {'violations': violations})
...
def user_ppe_violations(request):
    ...
    from .models import ViolationSnapshot
    violations = (ViolationSnapshot.objects
                  .filter(employee=user, confirmed=True)
                  .select_related('inspection', 'inspection__work_object')
                  .order_by('-created_at'))
...
def _available_tests_for_admin(admin, employee=None):
    ...
    if admin and not admin.is_global_admin and admin.department_id:
        ...
        qs = qs.filter(DQ(department_id=dept_id) | DQ(department__isnull=True))
    return qs
...
def admin_users_list(request):
    ...
    if search:
        users = users.filter(DQ(FIO__icontains=search) | DQ(login__icontains=search) | DQ(number__icontains=search))
    if status_filter == 'active':
        users = users.filter(isActive=True)
    elif status_filter == 'inactive':
        users = users.filter(isActive=False)
...
def admin_user_detail(request, user_id):
    from .forms import AdminUserProfileForm, AssignExistingTestsForm
    admin = _get_current_admin(request)
    user = get_object_or_404(_department_filtered_users(admin), id=user_id)
    if request.method == 'POST':
...
def admin_test_create(request):
    from .forms import CertificationTestForm
    user_role = request.session.get('user_role')
    user_id = request.session.get('user_id')
    if user_role not in ['admin', 'inspector']:
        messages.error(request, 'Доступ запрещён')
...
def admin_test_list(request):
    ...
    if user_role == 'admin':
        admin = _get_current_admin(request)
        if admin and not admin.is_global_admin and admin.department_id:
            tests = tests.filter(DQ(department_id=admin.department_id) | DQ(department__isnull=True))
    elif user_role == 'inspector':
        inspector = Inspector.objects.get(id=user_id)
        tests = tests.filter(DQ(created_by_inspector=inspector) | DQ(department_id=inspector.department_id) | DQ(department__isnull=True))
...
def medic_medical_check_create(request):
    ...
    if not _medic_required(request):
        return redirect('profiles:dashboard')
    medic = _get_current_employee(request)
    if request.method == 'POST':
        form = MedicalCheckForm(request.POST)
        if form.is_valid():
            check = form.save(commit=False)
        ...
        form = MedicalCheckForm()
    return render(request, 'profiles/medic/medical_check_form.html', {'form': form})
...
def medic_medical_check_create_for_employee(request, employee_id):
    ...
    if request.method == 'POST':
        ...
        form = MedicalCheckForm(initial={'employee': employee, 'checked_at': dj_timezone.now()})
    return render(request, 'profiles/medic/medical_check_form.html', {'form': form, 'employee': employee})
...
def inspector_violation_decision(request, pk: int, violation_id: int):
    ...
    if decision == 'yes':
        violation.confirmed = True
        violation.save(update_fields=['employee', 'comment', 'reviewed_at', 'confirmed'])
        ...
        if violation.employee_id:
            Sanction.objects.create(
                ...
                comment=f'[VIDEO:{violation.id}] {violation.comment}'.strip(),
                status='active',
            )
        messages.success(request, 'Нарушение подтверждено')
    else:
        violation.confirmed = False
        violation.save(update_fields=['employee', 'comment', 'reviewed_at', 'confirmed'])
...
def dashboard(request):
    ...
    if user_role == 'medic':
        ...
        today_checks = MedicalCheck.objects.filter(check_date=today)
        context['medic'] = medic
        context['medic_stats'] = {'checks_total': today_checks.count(), 'fit_total': today_checks.filter(decision='fit').count(), 'unfit_total': today_checks.filter(decision='unfit').count(), 'sick_total': SickLeave.objects.filter(status='active', start_date__lte=today, end_date__gte=today).count()}
        return render(request, 'profiles/medic/dashboard.html', context)
    if user_role == 'admin':
        admin = Admin.objects.filter(id=user_id).select_related('department').first()
        context['admin_obj'] = admin
        context['dashboard_stats'] = {'users_total': _department_filtered_users(admin).count(), 'users_pending': _department_filtered_users(admin).filter(isActive=False).count(), 'work_open': _department_filtered_work_objects(admin).exclude(status__in=['completed', 'cancelled']).count(), 'work_overdue': _department_filtered_work_objects(admin).filter(status='overdue').count()}
        return render(request, 'profiles/dashboard.html', context)
